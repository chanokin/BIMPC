...
#define GAME_WIDTH  32//160
#define GAME_HEIGHT 32//128
...
//i coordinate is shifted by ceil(log2(GAME_HEIGHT))+1
  const uint32_t spike_key = key | (SPECIAL_EVENT_MAX + (i << 6) + (j << 1) + colour_bit);
...
void timer_callback(uint ticks, uint dummy)
{
  ...
  {
    //log_info(" simulation ended, infinite run=%d",infinite_run);			
    spin1_pause();//pause command added for finite time simulations
    return;
  }
  // Otherwise
  else
