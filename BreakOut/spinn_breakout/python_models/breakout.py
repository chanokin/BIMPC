...
class Breakout(
	...
	numx=np.ceil(np.log2(self.WIDTH_PIXELS))
        numy=np.ceil(np.log2(self.HEIGHT_PIXELS))
	numcol=np.ceil(np.log2(self.COLOUR_BITS))

        return int(2**(numx+numy+numcol)+2) #2 + (256 * 256 * 2)
